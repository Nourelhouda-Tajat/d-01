...
void ajout_joueur()
{
    ...
    scanf("%d", &c); // ajout boucle do while
    switch (c)
    {
    case 1:
...
void affiche_poste_ordre()
{
    ...
    affiche_ordre();
    printf("Entrer le poste voulue afficher ses joueurs : ");
    scanf("%s", poste_voulu);

    for (i = 0; i < cmpt; i++)
    {
        if (strcmp(equipe[i].poste, poste_voulu) == 0)
        {
            printf("|| %d  || %s  || %s  ||%d  || %s  || %d || %d ||\n", equipe[i].id, equipe[i].nom, equipe[i].prenom, equipe[i].n_maillot, equipe[i].poste, equipe[i].age, equipe[i].but);
        }
    }
...
void affichage_menu()
{
    ...
    do
    {
        ...
        printf("3. Affichage des joueurs par id \n");
        printf("4. Affichage par poste de joueur\n");
        ...
        case 2:
            ...
            break;
        case 3:
            printf("*************La liste des joueurs par id***************\n ");
            affiche_id();
            affichage();
            break;
        case 4:
            printf("*************La liste des joueurs par poste***************\n ");
            affiche_poste_ordre();
            break;
        case 5:
            printf("Retour vers le menu principal");
...
void moyen_age()
{
    ...
    printf("L age moyen des joureursde l equipe est : %d", total_age / (cmpt));
}
void cmp_but()
{
    int i, n, compteur;
...
void joueur_age_jeune()
{
    ...
    for (i = 1; i < cmpt; i++)
    {
        if (equipe[i].age > equipe[max].age)
        {
            max = i;
        }
    }
